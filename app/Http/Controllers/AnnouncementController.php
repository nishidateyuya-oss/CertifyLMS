...
class AnnouncementController extends Controller
{
    ...
    $targetStudents = match ($targetType) {
        // 全受講生 (受講中の受講生)
        AnnouncementTargetType::AllStudents, AnnouncementTargetType::AllStudents->value => 
            User::where('role', UserRole::Student->value)->where('status', UserStatus::InProgress->value)->get(),

        // 資格指定 (該当資格を受講中の受講生)
        AnnouncementTargetType::Certification, AnnouncementTargetType::Certification->value => 
            User::whereHas('enrollments', function ($query) use ($validated) {
                $query->where('certification_id', $validated['target_certification_id']);
            })->get(),

        // ユーザー指定 (指定された1人)
        AnnouncementTargetType::User, AnnouncementTargetType::User->value => 
            User::where('id', $validated['target_user_id'])->get(),

...
<x-card padding="md" shadow="sm">
    ...
    @can('create', [EnrollmentNote::class, $enrollment])
        <form novalidate method="POST" action="{{ route('enrollments.notes.store', $enrollment) }}"
            class="space-y-3 pb-4 border-b border-ink-100">
            @csrf
            <x-form.textarea name="body" label="新規メモ" :rows="3" :value="old('body')" :error="$errors->first('body')"
                :maxlength="2000" />
                ...
                <li class="p-3 rounded-md border border-ink-100">
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <div class="flex items-center gap-1">
                            @can('update', $note)
                                <x-link-button href="{{ route('enrollment-notes.edit', $note) }}" variant="ghost"
                                    size="sm">
                                    <x-icon name="pencil-square" class="w-4 h-4" />
                                </x-link-button>
                            @endcan
                            @can('delete', $note)
                                <form novalidate method="POST" action="{{ route('enrollment-notes.destroy', $note) }}"
